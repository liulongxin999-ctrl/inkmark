...
7. 左侧栏「复习」下面还有个 **AI**：一台独立的通用助手，写代码、润色、翻译都行（见下面「AI 助手」）

**带公式和插图的资料怎么读？** 把 `.md` 和 `images/` 文件夹打包成 `.zip` 拖进书库（或点「选择文件夹」）：
`$...$` 公式会**渲染成真正的数学排版**，插图会**真正显示**并可点击放大。
扫描版 PDF（没有文字层）需要先 OCR 转成 Markdown，再按上面的方式打包成 zip 导入——
具体怎么转，见 [docs/图片型PDF转文字指南.md](docs/图片型PDF转文字指南.md)。
...
- **AI 问答是可选功能，需要你自己配置服务商**（当前支持 DeepSeek）。它默认关闭；
  开启后，读书时的每次提问才会把「你选中的原文 + 它所在的段落 + 书名章节」发给你指定的服务商。
  **你的批注、笔记和术语的「我的理解」永远不发送**，API Key 也只存放在本机。
  左侧栏「AI」里的独立问答更干净：**只发送你在那里写下的内容**。
...
**读书时问** —— 选中读不懂的段落，直接问 AI；回答可以一键存成术语或笔记，
接进你已有的知识库和复习队列。

**随时问** —— 左侧栏「复习」下面的 **AI** 是一台独立的通用助手：写代码、润色文字、
...
完整说明见 [docs/AI助手使用说明.md](docs/AI助手使用说明.md)——怎么申请 Key、在哪配、
每次到底会发出什么、怎么彻底关掉。
...
墨读/
  ...
  示例/                     示例书，可直接拖进书库
  docs/图片型PDF转文字指南.md  扫描版 PDF 转成可批注文字的操作指南
  docs/AI助手使用说明.md     AI 助手怎么配、会发送什么、怎么关掉
  ...
  src/views/                library 书库 · notes 笔记工作台 · review 复习 · ai 独立 AI 工作台
  src/ui/                   shell 主题 / 弹窗 / 命令面板 · ai 阅读侧栏问答 · chatkit 聊天共用零件
  tests/                    自检与端到端测试
```
...
- 扫描版 PDF 没有文字层，正文无法提取，只能用「原版页」按原始排版阅读；想变成可批注的文字，需要先做 OCR 转换（[操作指南](docs/图片型PDF转文字指南.md)）。
